...
namespace FF7
{
	class SaveData
	{
		...
		public bool Open(String filename, bool force)
		{
			...
			if (force == false && CheckSum() != ReadNumber(0x09, 2))
			{
				mFileName = null;
				mBuffer = null;
				return false;
		...
		{
			uint crc = 0xFFFF;
			for (uint i = 0; i < 4336; i++)
			{
				uint value = ReadNumber(13 + i, 1);
				crc ^= value << 8;
				for (uint j = 0; j < 8; j++)
				{
					if((int)(crc & 0x8000) == 0)
			...
			return (ushort)(((int)crc ^ 0xFFFF) & 0xFFFF);
		}

		private uint CalcAddress(uint address)
		{
			return address + Adventure * 0x10F4;
		}
